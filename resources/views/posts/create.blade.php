...
<form action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3 mt-5">
        ...
        <input type="text" class="form-control @error('titolo') is-invalid @enderror" id="titolo" name="titolo" value="{{ old('titolo', $post->titolo ?? '') }}" required>
        @error('titolo')
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control @error('titolo') is-invalid @enderror" id="category" name="category" value="{{ old('category', $post->category ?? '') }}" required>
        @error('category')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
